this is the error <ERROR: permission denied for schema public
[ERROR]   Position: 32>, this was my inti file <-- Create database
...
ALTER DEFAULT PRIVILEGES IN SCHEMA blog
  GRANT USAGE, SELECT ON SEQUENCES TO bsapi_user;
>, what other permissions could i have granted, include the psql commands as a comment in init sql clearly stating that there is no need to delete database and run it once more with this <the actual command to run the init file>, instead just go to the terminal and manually add these permissions <the full command plus the grant permmissions> and success.